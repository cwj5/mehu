...
Update: 2026-03-27. GUI camera sync now honors backend `plot_up` orientation for both axis presets and custom viewpoints by passing `plot_up` through `App` into `Viewer3D` and applying it to camera-up vectors during synchronization. Integration coverage was extended to assert `plot_up` pass-through (`cameraPlotUp`) so `/UP` stays locked across script and GUI paths.
